...
def get_account_access_token_async(
    consts, arm_access_token, permission_type="Contributor", scope="Account", video_id=None
):
    """
    ...
    headers = {"Authorization": "Bearer " + arm_access_token, "Content-Type": "application/json"}

    url = (
        f"{consts.AzureResourceManager}/subscriptions/{consts.SubscriptionId}/resourceGroups/{consts.ResourceGroup}"
        + f"/providers/Microsoft.VideoIndexer/accounts/{consts.AccountName}/generateAccessToken?api-version={consts.ApiVersion}"
    )

    params = {"permissionType": permission_type, "scope": scope}
